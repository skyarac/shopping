...
public class ForderServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
		...
		if(status.equals("buy")){
			...
			request.getSession().setAttribute("forder",forderDaoImpl.addSorder(forder, sorder));
			...
			request.getSession().setAttribute("forder",forderDaoImpl.saveForder(forder));
		...
		//	System.out.println("总价 "+forder.getFtotal());
			...
			int gid=Integer.parseInt(request.getParameter("gid"));
			Forder forder = (Forder) request.getSession().getAttribute("forder");
			request.getSession().setAttribute("forder",forderDaoImpl.deleteSorder(forder, gid));
			response.sendRedirect("/shopping/users/showCar.jsp");
		}
	}
